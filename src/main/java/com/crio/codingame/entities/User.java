...
public class User extends BaseEntity {
    ...
    public boolean checkIfContestExists(Contest contest){
        List<String> contestsName = contests.stream().map(e -> e.getName()).collect(Collectors.toList());
        boolean isExist = contestsName.stream().anyMatch(element -> element.contains(contest.getName()));
        return isExist;
    }
