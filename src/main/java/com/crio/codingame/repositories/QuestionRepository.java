...
public class QuestionRepository implements IQuestionRepository {
    ...
    public List<Question> findAll() {
        Optional<String> result = questionMap.keySet().stream().findAny();
        ArrayList<Question> qList = new ArrayList<Question>(questionMap.values());
        if(result.isEmpty()){
            return qList;
        }
        return qList;
    ...
    public Optional<Question> findById(String id) {
        Optional<Question> result = questionMap.entrySet()
        .stream().filter(x->id.equals(x.getKey()))
    ...
    public List<Question> findAllQuestionLevelWise(Level level) {
        List<Question> contestList = questionMap.values().stream().filter(u->u.getLevel().equals(level)).collect(Collectors.toList());
        return contestList;
    }
