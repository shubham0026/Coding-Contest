...
public class ContestRepository implements IContestRepository {
    ...
    public List<Contest> findAll() {
        // Optional<String> result = contestMap.keySet().stream().findAny();
        
        // if(result.isEmpty()){
        //     throw new InvalidContestException();
        // }
       // ArrayList<Contest> contestList = (ArrayList<Contest>) contestMap.values();
    ...
    public List<Contest> findAllContestLevelWise(Level level) {
        // Optional<Contest> result = contestMap.values().stream().filter(u->u.getLevel().equals(level)).findAny();
        // if(result.isEmpty()){
        //     return Collections.emptyList();
        // }
        List<Contest> contestList = contestMap.values().stream().filter(u->u.getLevel().equals(level)).collect(Collectors.toList());
     return contestList;
    }
